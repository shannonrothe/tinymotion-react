const SINGLE_WORD_PROPERTIES = ["bg", "text", "border", "ring"];
...
class Animator {
  ...
  parseProperty(property) {
    ...
    if (
      SINGLE_WORD_PROPERTIES.includes(propertyParts[0]) &&
      propertyParts.length == 3
    ) {
      propertyNameParts = propertyParts.splice(0, 1);
    ...
    // other properties (w-16, shadow-lg)
      ...
      propertyNameParts = propertyParts.splice(0, propertyParts.length - 1);
    ...
    if (propertyNameParts[0] == "") {
      propertyName = propertyNameParts.splice(1).join("-");
      value = -value;
    }
  ...
  buildTimeline() {
    ...
    timeline.forEach((timeframe) => {
      let classList = Object.entries(timeframe).map(([propertyName, value]) => {
        if (value == null) {
          return propertyName;
